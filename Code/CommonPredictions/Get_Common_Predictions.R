...
#Read the files
RF <- read.table("Final_Ecoli_sRNARFTarget_predictions.txt", header = T, sep= "\t",stringsAsFactors = F)
IntaRNA <- read.table("Final_Ecoli_IntaRNA_predictions.txt", header = T, sep = "\t", stringsAsFactors = F)
dim(RF)
dim(IntaRNA)
